"""The six ``p2p-pursuit`` command handlers: peer, sim, replay, verify, smoke,
authorize.

Split out of :mod:`.cli`, which now holds only the argparse surface, so both
files stay inside the guidelines' 150-line limit (§3.2 - split, never compress).
Every handler stays a thin shell: the work belongs to :class:`~.sdk.PursuitSDK`
(guidelines ch. 4 - no business logic outside the SDK). stdout carries
machine-readable JSON only; human logs go to stderr.
"""
