...
def reference_records(sealed_records: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    to is exactly what we played. Requires the records to have been sealed
    under the reference dialect, or the commits will not match what we sent.
    """
    out = []
    for sealed in sealed_records:
        payload = {k: v for k, v in sealed.items() if k != "nonce"}
        out.append({"payload": payload, "nonce": sealed["nonce"],
                    "commit": reference_commit(payload, sealed["nonce"])})
    return out
